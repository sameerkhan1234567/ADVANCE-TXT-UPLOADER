...
async def upload(bot: Client, m: Message):
    ...
    try:
        ...
        arg = 1
    await editable.edit("📚 𝗘𝗻𝘁𝗲𝗿 𝗬𝗼𝘂�_r 𝗕𝗮𝘁𝗰𝗵 𝗡𝗮𝗺𝗲 📚\n\n🦠 𝗦𝗲𝗻𝗱 `1` 𝗙𝗼𝗿 𝗨𝘀𝗲 𝗗𝗲𝗳𝗮𝘂𝗹𝘁 🦠")
    ...
    if raw_text4 == '3':
        MR = ""  # Set to empty string for non-PW uploads
    else:
        MR = raw_text4

    # Prompt for x-access-token for transcoded-videos.classx.co.in
    await editable.edit("**Enter x-access-token for ClassX URLs (or send `skip` to try without a token):**")
    input_token: Message = await bot.listen(editable.chat.id)
    access_token = input_token.text if input_token.text != 'skip' else None
    await input_token.delete(True)

    await editable.edit("𝗡𝗼𝘄 𝗦𝗲𝗻𝗱 𝗧𝗵𝗲 𝗧𝗵𝘂𝗺𝗯 𝗨𝗿𝗹 𝗘𝗴 » https://graph.org/file/13a89d77002442255efad-989ac290c1b3f13b44.jpg\n\n𝗢𝗿 𝗜𝗳 𝗗𝗼𝗻'𝘁 𝗪𝗮𝗻𝘁 𝗧𝗵𝘂𝗺𝗯𝗻𝗮𝗶𝗹 𝗦𝗲𝗻�_d = 𝗻𝗼")
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
                ...
                headers = {
                    ...
                    'x-access-token': access_token if access_token else 'eyJjb3Vyc2VJZCI6IjQ1NjY4NyIsInR1dG9ySWQiOm51bGwsIm9yZ0lkIjo0ODA2MTksImNhdGVnb3J5SWQiOm51bGx9',
                ...
                try:
                    async with ClientSession() as session:
                        async with session.get('https://api.classplusapp.com/cams/uploader/video/jw-signed-url', headers=headers, params=params) as resp:
                            if resp.status == 200:
                                url = (await resp.json())['url']
                            elif resp.status in (401, 403):
                                await m.reply_text(f"Link expired or unauthorized: {url}. Please provide a valid x-access-token.")
                                logging.error(f"Unauthorized access for {url}: {resp.status} {resp.reason}")
                                continue
                            else:
                                await m.reply_text(f"Failed to fetch signed URL: {resp.status} {resp.reason}")
                                logging.error(f"Failed to fetch signed URL for {url}: {resp.status} {resp.reason}")
                                continue
                except Exception as e:
                    await m.reply_text(f"Error fetching signed URL for ClassX: {str(e)}")
                    logging.error(f"Error fetching signed URL for {url}: {str(e)}")
                    continue
                    ...
                    async with session.get(url) as resp:
                        if resp.status == 200:
                            ...
                            if "#EXT-X-KEY" in m3u8_content:
                                await m.reply_text(f"DRM-protected video detected: {url}. Cannot download without decryption keys.")
                                logging.error(f"DRM-protected video: {url}")
                                continue
                        elif resp.status in (401, 403):
                            await m.reply_text(f"Link expired or unauthorized: {url}. Please provide a valid x-access-token.")
                            logging.error(f"Unauthorized access for {url}: {resp.status} {resp.reason}")
                            ...
                            await m.reply_text(f"Failed to access URL: {resp.status} {resp.reason}")
                            logging.error(f"Failed to access {url}: {resp.status} {resp.reason}")
                            continue
                cmd = f'yt-dlp --cookies {COOKIES_FILE_PATH} -f "{ytf}" "{url}" -o "{name}.mp4"'
                ...
                if "drive" in url:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            logging.error(f"Failed to download PDF {url}: {response.status_code} {response.reason}")
                    except FloodWait as e:
                        await m.reply_text(str(e))
                        time.sleep(e.x)
                        continue
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            logging.error(f"Failed to download Image {url}: {response.status_code} {response.reason}")
                    except FloodWait as e:
                        await m.reply_text(str(e))
                        await asyncio.sleep(2)
                        continue
        ...
        logging.error(f"Unexpected error in upload command: {str(e)}")
    
    await m.reply_text(f"`✨𝗕𝗔𝗧𝗖𝗛 𝗦𝗨𝗠𝗠𝗔𝗥𝗬✨\n\n"
                       f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
                       f"📛𝗜𝗻𝗱𝗲𝘅 �_R𝗮𝗻𝗴𝗲 » ({raw_text} to {len(links)})\n"
                       f"📚𝗕𝗮𝘁𝗰𝗵 𝗡𝗮𝗺𝗲 » {b_name}\n\n"
                       f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
